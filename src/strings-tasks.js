...
function concatenateStrings(value1, value2) {
  return [].concat(value1, value2).join('');
}
...
function removeFirstOccurrences(str, value) {
  ...
  const str2 = str.slice(index, index + value.length);
...
function removeLastOccurrences(str, value) {
  ...
  const sub1 = str.slice(index, index + value.length);
  const sub2 = sub1.split('').reverse().join('');
  const str2 = str.split('').reverse().join('');
  return str2.replace(sub2, '').split('').reverse().join('');
}
...
function isPalindrome(str) {
  const str2 = str.replaceAll(/[^a-zа-яё]/gi, '');
  let flag = true;
  for (let i = 0, j = str2.length - 1; i < str2.length; i += 1, j -= 1) {
...
function encodeToRot13(str) {
  const str1 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'.split('');
  const str2 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'.split('');
